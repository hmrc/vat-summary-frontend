...
class PaymentsOnAccountService @Inject()(standingRequestsConnector: StandingRequestsConnector)
  ...
  def getPaymentsOnAccounts(regime: String, taxpayerIdType: String, taxpayerIdValue: String)
                           (implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Option[StandingRequest]] = {

    standingRequestsConnector.getStandingRequests(regime, taxpayerIdType, taxpayerIdValue).map {
      case Right(standingRequest) =>
        logger.info(s"Successfully retrieved payments on account for $taxpayerIdValue")
